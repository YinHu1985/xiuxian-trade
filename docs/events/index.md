...
| 委托类型 | 完成条件 | 适用场景 |
|---------|---------|---------|
| `purchase` | 玩家货仓中有指定商品 | 采购任务 |
| `deliver` | 玩家背包中有指定信件物品 | 送信任务 |
| `trade` (buy) | 玩家背包中有指定天材地宝 | 出售给NPC |
| `trade` (sell) | 玩家支付灵石 | NPC出售给玩家 |
| `relic` | 玩家背包中有指定 relicId 的物品 | 法宝寻回 |
| `item_delivery` | 玩家背包中有指定名称的物品（消耗） | 通用交付任务 |
...
当前系统已支持：
- ✅ 事件条件（flag、物品、回合、节点类型等）
- ✅ 事件效果（flag、灵石、物品、船员、飞舟等）
- ✅ 委托系统（接取、完成、检查物品）
- ✅ `-` `relic` 委托类型检查背包物品

需要补充：
- [ ] `add_quest` 效果类型：事件中创建委托
- [ ] `item_delivery` 委托类型：按物品名检查并消耗
- [ ] `quest_complete` 触发接续事件（trigger 已定义但未接入引擎循环）
- [ ] `action` 触发的事件筛选逻辑（actionType 过滤）
